...
export function TradeVote({id,left,right,summary,onSaved}:{id:string;left:string;right:string;summary?:{average:number;votes:number};onSaved:(data:unknown)=>void}) {
  ...
  return <div className="vote-shell"><button className="vote-toggle" aria-expanded={open} aria-controls={`trade-vote-${id}`} onClick={()=>setOpen(x=>!x)}><span>{open?"Hide vote":"Vote"}</span>{summary&&<small>{summary.votes} vote{summary.votes===1?"":"s"}</small>}</button>{open&&<div className="vote-box" id={`trade-vote-${id}`}><div className="vote-title"><strong>Who won?</strong>{summary&&<span>{summary.average===0?"Even":summary.average<0?`${left} ${Math.abs(summary.average)}%`:`${right} ${summary.average}%`} · {summary.votes} vote{summary.votes===1?"":"s"}</span>}</div><input aria-label={`Rate trade ${id}`} type="range" min="-100" max="100" step="5" value={value} onChange={e=>{setValue(Number(e.target.value));setLocked(false)}}/><div className="range-labels"><span>{left}</span><b>{label}</b><span>{right}</span></div><button onClick={save} disabled={saving}>{saving?"Locking…":locked?"Locked ✓":"Lock choice"}</button></div>}</div>;
}
...
export function RoastVote({id,summary,onSaved}:{id:string;summary?:{average:number;votes:number};onSaved:(data:unknown)=>void}) {
  ...
  return <div className="vote-shell roast-vote-shell"><button className="vote-toggle" aria-expanded={open} aria-controls={`roast-vote-${id}`} onClick={()=>setOpen(x=>!x)}><span>{open?"Hide vote":"Vote"}</span>{summary&&<small>{summary.average.toFixed(1)} ★ · {summary.votes}</small>}</button>{open&&<div className="star-vote" id={`roast-vote-${id}`}><div><strong>Rate this roast</strong>{summary&&<span>{summary.average.toFixed(1)} ★ · {summary.votes} rating{summary.votes===1?"":"s"}</span>}</div><div className="stars" role="radiogroup" aria-label={`Rate roast ${id}`}>{[1,2,3,4,5].map(star=><button type="button" role="radio" aria-checked={value===star} aria-label={`${star} star${star===1?"":"s"}`} className={star<=value?"chosen":""} key={star} onClick={()=>{setValue(star);setLocked(false)}}>★</button>)}</div><button className="lock-rating" onClick={save} disabled={!value||saving}>{saving?"Locking…":locked?"Locked ✓":"Lock rating"}</button></div>}</div>;
}
